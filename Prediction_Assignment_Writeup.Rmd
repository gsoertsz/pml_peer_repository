...
Using data from the Human Activity Recognition study, a classifier was trained to identify the type of activity given a number of input measures. The classifier was trained using a randomForest algorithm, using a resampling approach based on repeated cross validation based on training data of a chosen strata, and column projection.

The trained models were evaluated using overall accuracy and ROC performance reflecting on Strong accuracy results were achieved for class A and B, with moderate-to-good accuracy results achieved for the other classes. 
...
print.xtable(xtable(c_rf_tq), type="html")
```
...
result_rf.probs <- predict(fit_rf, newdata=cross_validation, type="prob")
result_rf.base <- predict(fit_rf, newdata = cross_validation, type="raw")
g_rf <- evaluate(cross_validation, probs=result_rf.probs)
g_rf

c_rf <- confusionMatrix(result_rf.base, cross_validation$classe)

c_acc <- c_rf$overall[[1]]
```
...
acc <- data.frame(Strata = strata, Accuracy = accuracies)
